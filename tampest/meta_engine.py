    ...
    def _solve(
        ...
        while True:
            ...
            if self.engine.supported_kind().has_object_fluents():
                ...
                res = self.engine.solve(
                    comp_problem, timeout=timeout, output_stream=output_stream
                )
                plan = (
                    None
                    if res.plan is None
                    else res.plan.replace_action_instances(
                ...
                if is_valid:
                    return PlanGenerationResult(
                        ...
                        plan.replace_action_instances(
                            partial(_replace_action, orig_problem=orig_problem)
                        ),
                        self.name,
...
def _replace_action(action_instance, orig_problem):
    return ActionInstance(
        orig_problem.action(action_instance.action.name),
