...
class Map3D:
    ...
    def __eq__(self, oth: object) -> bool:
        if isinstance(oth, trimesh.Trimesh):
            return all(
                trimesh.comparison.identifier_simple(self._mesh)
                == trimesh.comparison.identifier_simple(oth.mesh)
            )
        else:
            return False
    ...
    def set_from_file(self, yaml_file: str):
        ...
        with open(yaml_file) as f:
            ...
            self._mesh = self.get_mesh(
                os.path.join(os.path.dirname(yaml_file), data["mesh"])
            )
    ...
    def set_from_file(self, yaml_file: str):
        ...
        with open(yaml_file) as f:
            ...
            self._image = self.get_image(
                os.path.join(os.path.dirname(yaml_file), data["image"])
            )
