...
class Maze:
    ...
    def get_problem(self, dim: string, nd: int, nc: int):
        ...
        if (
            ...
            RobotConfig = ConfigurationType(
                "robot_config", occ_map, robot_configuration_kind
            )  # (x, y, yaw)
            robot_configs = {
        ...
        if nd > 0:
            ...
            if door_configuration_kind != robot_configuration_kind:
                raise TypeError(
                    "Doors and Robots must move in an environment of same dimension."
                )
        ...
        # add actions
        move_robot = InstantaneousMotionAction(
        ...
        if nd > 0:
            move_robot.add_motion_constraint(
                Waypoints(r, c_from, [c_to], {d: door_at(d) for d in doors})
            )
        else:
    ...
    def get_problem_from_config(
        ...
        occ_map = OccupancyMap(
            os.path.join(os.path.dirname(domain_file), domain_data["world"]["file"]),
        ...
        for r in involved_robots:
            ...
            start_config = ConfigurationObject(
                r["start"], RobotConfig, robot_configs[r["start"]]
            )
            ...
            for g in r["goals"]:
                if (
                    problem.has_object(g)
                    and problem.object(g).configuration != robot_configs[g]
                ):
                    raise ValueError(
                        f"Location {g} already defined with different params."
                    )
        ...
        for d in involved_doors:
            for c in door_configs:
                if d["name"] == c["name"]:
                    # door
                    if problem.has_object(c["name"]) and (
                        problem.object(c["name"]).footprint != door_footprint
                        or problem.object(c["name"]).foomotion_model
                        != door_motion_model
                        or problem.object(c["name"]).paramters != door_params
                    ):
                        raise ValueError(
                            f"Door {c['name']} already defined with different params."
                        )
                    ...
                    # open config
                    if problem.has_object(c["name"] + "_open") and problem.object(
                        c["name"] + "_open"
                    ).configuration != SE2(*ast.literal_eval(c["open"])):
                        raise ValueError(
                            f"Open configuration {c['name']} already defined with different params."
                        )
                    ...
                    if (
                        ...
                        open_config = ConfigurationObject(
                            c["name"] + "_open",
                            DoorConfig,
                            SE2(*ast.literal_eval(c["open"])),
                        )
                    elif robot_motion_model == MotionModels.SE3:
                        open_config = ConfigurationObject(
                            c["name"] + "_open",
                            DoorConfig,
                            SE3(*ast.literal_eval(c["open"])),
                        )
                    ...
                    # close configs
                    # open config
                    if problem.has_object(c["name"] + "_closed") and problem.object(
                        c["name"] + "_closed"
                    ).configuration != SE2(*ast.literal_eval(c["close"])):
                        raise ValueError(
                            f"Open configuration {c['name']} already defined with different params."
                        )
                    ...
                    if (
                        ...
                        close_config = ConfigurationObject(
                            c["name"] + "_closed",
                            DoorConfig,
                            SE2(*ast.literal_eval(c["close"])),
                        )
                    elif robot_motion_model == MotionModels.SE3:
                        close_config = ConfigurationObject(
                            c["name"] + "_closed",
                            DoorConfig,
                            SE3(*ast.literal_eval(c["close"])),
                        )
