...
class Doors:
    ...
    def get_random_configs(
        self, n: int, minx: float, maxx: float, miny: float, maxy: float
    ) -> List[Tuple[float, ...]]:
        configs = []
        for i in range(n):
    ...
    def get_closed_locs(self, n: int) -> Dict[str, Tuple[float, ...]]:
        locs = {}
        for i in range(n):
            locs[f"c{i}"] = SE2(16.3 + 10.0 * i, 15.0, 0.0)
        return locs

    def get_open_locs(self, n: int) -> Dict[str, Tuple[float, ...]]:
        locs = {}
        for i in range(n):
            locs[f"o{i}"] = SE2(16.3 + 10.0 * i, 21.0, 0.0)
        return locs

    def get_button_locs(self, n: int) -> Dict[str, Tuple[float, ...]]:
        locs = {}
        for i in range(n):
            locs[f"b{i}"] = SE2(12.2 + 10.0 * i, 15.0, -math.pi / 2.0)
        return locs

    def get_configs_from_file(
        region: int, n: int, filename: str
    ...
    def get_problem(
        self, n_door: int, *, c0: Optional[int] = 0, c1: Optional[int] = 0
    ):  # co = # of configurations left - c1 = # of configurations right

        occ_map = OccupancyMap(self.get_map(n_door), SE2(0, 0, 0))
        ...
        start_config = ConfigurationObject(
            "s", RobotConfig, SE2(3.0, 26.0, -math.pi / 2.0)
        )
        ...
        if c0 != 0 or c1 != 0:
            ...
            connection_configs = [
                ConfigurationObject("t%s" % i, RobotConfig, SE2(*connections[i]))
        ...
        goal_config = ConfigurationObject(
            "g", RobotConfig, SE2(26.0 + 10.0 * (n_door - 1), 26.0, math.pi / 2.0)
        )
        ...
        open_locs = [
            ConfigurationObject(k, DoorConfig, v)
            for k, v in self.get_open_locs(n_door).items()
        ]
        close_locs = [
            ConfigurationObject(k, DoorConfig, v)
            for k, v in self.get_closed_locs(n_door).items()
        ]
        button_locs = [
            ConfigurationObject(k, RobotConfig, v)
            for k, v in self.get_button_locs(n_door).items()
        ]

        move_action = InstantaneousMotionAction(
        ...
        if connection_configs:
            ...
            all_conn = itertools.permutations(
                connection_configs + [start_config, goal_config] + button_locs, 2
            )
            for c in all_conn:
