...
class Delivery:
    ...
    def get_problem(
        ...
        if (
            ...
            or robot_motion_model == MotionModels.SE2
        ):
            occ_map = OccupancyMap(
                os.path.join(
                    os.path.dirname(domain_file), domain_data["world"]["file"]
                ),
                SE2(*ast.literal_eval(domain_data["world"]["ref_frame"])),
            )
            ...
            occ_map = OccupancyMap(
                ...
                SE3(*ast.literal_eval(domain_data["world"]["ref_frame"])),
            )
        else:
        ...
        if "model" in domain_data["robot"]:
            robot_model = os.path.join(
                os.path.dirname(domain_file), domain_data["robot"]["model"]
            )
        ...
        if (
            ...
            robot_configuration_kind = ConfigurationKind.SE2
            Config = ConfigurationType(
                "config", occ_map, robot_configuration_kind
            )  # (x, y, yaw)
            robot_configs = {
                l["name"]: SE2(*ast.literal_eval(l["pose"]))
                for l in domain_data["robot_configs"]
            }
        elif robot_motion_model == MotionModels.SE3:
            robot_configuration_kind = ConfigurationKind.SE3
            Config = ConfigurationType(
                "config", occ_map, robot_configuration_kind
            )  # (x, y, z, rx, ry, rz, angle)
            robot_configs = {
                l["name"]: SE3(*ast.literal_eval(l["pose"]))
                for l in domain_data["robot_configs"]
            }
        else:
            raise NotImplementedError
        ...
        if nd > 0:
            ...
            move.add_motion_constraint(
                Waypoints(r_move, c_from, [c_to], {d: door_at(d) for d in doors})
            )
        else:
        ...
        robot = MovableObject(
            ...
            geometric_model=robot_model,
            motion_model=robot_motion_model,
            motion_parameters=robot_params,
        )
        ...
        if nd > 0:
            ...
            if "model" in domain_data["door"]:
                door_model = os.path.join(
                    os.path.dirname(domain_file), domain_data["door"]["model"]
                )
            ...
            # door configuration
            if (
                door_motion_model == MotionModels.REEDSSHEPP
                or door_motion_model == MotionModels.SE2
            ):
                door_configuration_kind = ConfigurationKind.SE2
                DoorConfig = ConfigurationType(
                    "door_config", occ_map, door_configuration_kind
                )  # (x, y, yaw)
                doors_configs = {
                    l["name"]: (
                        SE2(*ast.literal_eval(l["open"])),
                        SE2(*ast.literal_eval(l["close"])),
                    )
                    for l in domain_data["door_configs"]
                }
            elif door_motion_model == MotionModels.SE3:
                door_configuration_kind = ConfigurationKind.SE3
                DoorConfig = ConfigurationType(
                    "door_config", occ_map, door_configuration_kind
                )  # (x, y, z, rx, ry, rz, angle)
                doors_configs = {
            ...
            if door_configuration_kind != robot_configuration_kind:
                raise TypeError(
                    "Robots and Doors must act in a setup with the same dimension."
                )
            ...
            open_door = InstantaneousMotionAction(
                f"open", d=Door, c_close=DoorConfig, c_open=DoorConfig
            )
            ...
            with open(doors_file) as f:
                ...
                for d in doors_data[nd]:
                    ...
                    door = MovableObject(
                        ...
                        geometric_model=door_model,
                        motion_model=door_motion_model,
                        motion_parameters=door_params,
                    )
                    problem.add_object(door)
                    doors.append(door)

                    open_config = ConfigurationObject(
                        d + "_open", DoorConfig, door_open
                    )
                    problem.add_object(open_config)
                    problem.set_initial_value(opened(door), open_config)

                    close_config = ConfigurationObject(
                        d + "_closed", DoorConfig, door_close
                    )
        ...
        if (nr + ng) > 0:
            ...
            delivery_config = Fluent(
                "delivery_config", BoolType(), config1=Config, config2=Config
            )
            unloading_config = Fluent(
                "unloading_config", BoolType(), parcel=Parcel, config=Config
            )
            ...
            if (
                ...
                available_parcel_configs = {
                    l["name"]: SE2(*ast.literal_eval(l["pose"]))
                    for l in domain_data["parcel_configs"]
                }
            elif robot_motion_model == MotionModels.SE3:
                available_parcel_configs = {
                    l["name"]: SE3(*ast.literal_eval(l["pose"]))
            ...
            available_unloading_configs = {
                l["color"]: l["config"] for l in domain_data["unload_configs"]
            }
            ...
            with open(parcels_file) as f:
                ...
                for color, targets in parcel_data[
                    str((nr, ng, nr_del, ng_del))
                ].items():
                    n = 0
                    for t in targets:
                        parcel_config = ConfigurationObject(
                            t, Config, available_parcel_configs[t]
                        )
                        ...
                        del_config = ConfigurationObject(
                            delivery_configs[t],
                            Config,
                            robot_configs[delivery_configs[t]],
                        )
                        ...
                        problem.set_initial_value(
                            parcel_at(parcel, parcel_config), True
                        )
                        problem.set_initial_value(is_free(parcel_config), False)

                        problem.set_initial_value(
                            delivery_config(del_config, parcel_config), True
                        )
                ...
                for color, n in p_color.items():
                    ...
                    for elem in unloading_configs:
                        if (
                            problem.has_object(elem)
                            and problem.object(elem).configuration
                            != available_parcel_configs[elem]
                        ):
                            raise ValueError(
                                f"Configuration {elem} already defined with different params."
                            )

                        un_config = ConfigurationObject(
                            elem, Config, available_parcel_configs[elem]
                        )
                        ...
                        if not delivery_configs[elem]:
                            raise ValueError(
                                f"Delivery config of {elem} does not exists."
                            )

                        del_config = ConfigurationObject(
                            delivery_configs[elem],
                            Config,
                            robot_configs[delivery_configs[elem]],
                        )
                        ...
                        problem.set_initial_value(
                            delivery_config(del_config, un_config), True
                        )

                        for p, p_color in parcels.items():
                            if p_color == color:
                                problem.set_initial_value(
                                    unloading_config(p, del_config), True
                                )

        return problem

    def get_problem_from_config(
        self, dim: string, domain_file: string, problem_file: string
    ):
        ...
        if (
            ...
            occ_map = OccupancyMap(
                ...
                SE2(domain_data["world"]["ref_frame"]),
            )
        elif robot_motion_model == MotionModels.SE3:
            occ_map = OccupancyMap(
                os.path.join(
                ...
                SE3(domain_data["world"]["ref_frame"]),
            )
        else:
            raise NotImplementedError
        ...
        if door_configuration_kind != robot_configuration_kind:
            raise TypeError("Robots and Doors must act in the same dimension.")
        ...
        move = InstantaneousMotionAction(
            f"move", robot=Robot, c_from=Config, c_to=Config
        )
        ...
        for r in involved_robots:
            ...
            # robot
            if problem.has_object(r["name"]) and (
                problem.object(r["name"]).footprint != robot_footprint
                or problem.object(r["name"]).foomotion_model != robot_motion_model
                or problem.object(r["name"]).paramters != robot_params
            ):
                raise ValueError(
                    f"Robot {r['name']} already defined with different params."
                )
            ...
            # start config
            if (
                problem.has_object(r["start"])
                and problem.object(r["start"]).configuration
                != available_robot_configs[r["start"]]
            ):
                raise ValueError(
                    f"Location {r['start']} already defined with different params."
                )

            start_config = ConfigurationObject(
                r["start"], Config, available_robot_configs[r["start"]]
            )
        ...
        for d in involved_doors:
            ...
            # door
            if problem.has_object(d["name"]) and (
                problem.object(d["name"]).footprint != door_footprint
                or problem.object(d["name"]).foomotion_model != door_motion_model
                or problem.object(d["name"]).paramters != door_params
            ):
                raise ValueError(
                    f"Door {d['name']} already defined with different params."
                )
            ...
            # open config
            if (
                problem.has_object(d["name"] + "_open")
                and problem.object(d["name"] + "_open").configuration
                != door_configs[d["name"]][0]
            ):
                raise ValueError(
                    f"Open configuration {d['name']}_open already defined with different params."
                )

            open_config = ConfigurationObject(
                d["name"] + "_open", DoorConfig, door_configs[d["name"]][0]
            )
            ...
            # close configs
            if (
                problem.has_object(d["name"] + "_closed")
                and problem.object(d["name"] + "_closed").configuration
                != door_configs[d["name"]][1]
            ):
                raise ValueError(
                    f"Open configuration {d['name']}_close already defined with different params."
                )

            close_config = ConfigurationObject(
                d["name"] + "_closed", DoorConfig, door_configs[d["name"]][1]
            )
        ...
        for p in involved_parcels:
            ...
            if (
                problem.has_object(p["start"])
                and problem.object(p["start"]).configuration
                != available_parcel_configs[p["start"]]
            ):
                raise ValueError(
                    f"Configuration {p['start']} already defined with different params."
                )

            parcel_config = ConfigurationObject(
                p["start"], Config, available_parcel_configs[p["start"]]
            )
            ...
            del_config = ConfigurationObject(
                delivery_configs[p["start"]],
                Config,
                available_robot_configs[delivery_configs[p["start"]]],
            )
        ...
        for color, n in p_color.items():
            ...
            for elem in unloading_configs:
                ...
                del_config = ConfigurationObject(
                    delivery_configs[elem],
                    Config,
                    available_robot_configs[delivery_configs[elem]],
                )
                ...
                for p in involved_parcels:
                    if p["color"] == color:
                        problem.set_initial_value(
                            unloading_config(problem.object(p["name"]), del_config),
                            True,
                        )
