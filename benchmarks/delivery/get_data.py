...
def main():
    ...
    for n_r, n_g in parcels:
        ...
        for i in n_red_delivered:
            delivered_red = unloading_red_configs[
                :i
            ]  # random.sample(unloading_red_configs, i)
            for j in n_green_delivered:
                delivered_green = unloading_green_configs[
                    :j
                ]  # random.sample(unloading_green_configs, j)

                reds = (
                    random.sample(parcel_configs, n_r - len(delivered_red))
                    + delivered_red
                )
                greens = (
                    random.sample(
                        list(set(parcel_configs) - (set(reds) - set(delivered_red))),
                        n_g - len(delivered_green),
                    )
                    + delivered_green
                )
