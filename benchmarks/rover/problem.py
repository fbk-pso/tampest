...
class Rover:
    ...
    def get_problem(self, dim: string, d: int, c: int):
        ...
        if dim == "2D":
            Waypoint = ConfigurationType("waypoint", occ_map, ConfigurationKind.SE2)
            DoorConfig = ConfigurationType(
                "door_config", occ_map, ConfigurationKind.SE2
            )
        if dim == "3D":
            Waypoint = ConfigurationType("waypoint", occ_map, ConfigurationKind.SE3)
            DoorConfig = ConfigurationType(
                "door_config", occ_map, ConfigurationKind.SE3
            )
        ...
        equipped_for_soil_analysis = Fluent(
            "equipped_for_soil_analysis", BoolType(), r=Rover
        )
        equipped_for_rock_analysis = Fluent(
            "equipped_for_rock_analysis", BoolType(), r=Rover
        )
        equipped_for_imaging = Fluent("equipped_for_imaging", BoolType(), r=Rover)
        empty = Fluent("empty", BoolType(), s=Store)
        have_rock_analysis = Fluent(
            "have_rock_analysis", BoolType(), r=Rover, w=Waypoint
        )
        have_soil_analysis = Fluent(
            "have_soil_analysis", BoolType(), r=Rover, w=Waypoint
        )
        ...
        communicated_soil_data = Fluent(
            "communicated_soil_data", BoolType(), w=Waypoint
        )
        communicated_rock_data = Fluent(
            "communicated_rock_data", BoolType(), w=Waypoint
        )
        communicated_image_data = Fluent(
            "communicated_image_data", BoolType(), o=Objective, m=Mode
        )
        ...
        calibration_target = Fluent(
            "calibration_target", BoolType(), c=Camera, o=Objective
        )
        ...
        calibrate = InstantaneousAction(
            "calibrate", r=Rover, c=Camera, o=Objective, y=Waypoint
        )
        ...
        take_image = InstantaneousAction(
            "take_image", r=Rover, w=Waypoint, o=Objective, c=Camera, m=Mode
        )
        ...
        take_image.add_effect(
            have_image(r_take_image, o_take_image, m_take_image), True
        )
        take_image.add_effect(calibrated(c_take_image, r_take_image), False)

        communicate_soil_data = InstantaneousAction(
        ...
        communicate_soil_data.add_precondition(
            Equals(at(r_communicated_soil_data), y_communicated_soil_data)
        )
        communicate_soil_data.add_precondition(
            Equals(at_lander(l_communicated_soil_data), z_communicated_soil_data)
        )
        communicate_soil_data.add_precondition(
            have_soil_analysis(r_communicated_soil_data, x_communicated_soil_data)
        )
        communicate_soil_data.add_precondition(
            visible(y_communicated_soil_data, z_communicated_soil_data)
        )
        ...
        communicate_soil_data.add_effect(
            communicated_soil_data(x_communicated_soil_data), True
        )

        communicate_rock_data = InstantaneousAction(
        ...
        communicate_rock_data.add_precondition(
            Equals(at(r_communicate_rock_data), y_communicated_rock_data)
        )
        communicate_rock_data.add_precondition(
            Equals(at_lander(l_communicate_rock_data), z_communicated_rock_data)
        )
        communicate_rock_data.add_precondition(
            have_rock_analysis(r_communicate_rock_data, x_communicate_rock_data)
        )
        communicate_rock_data.add_precondition(
            visible(y_communicated_rock_data, z_communicated_rock_data)
        )
        ...
        communicate_rock_data.add_effect(
            communicated_rock_data(x_communicate_rock_data), True
        )

        communicate_image_data = InstantaneousAction(
        ...
        communicate_image_data.add_precondition(
            Equals(at(r_communicate_image_data), x_communicate_image_data)
        )
        communicate_image_data.add_precondition(
            Equals(at_lander(l_communicate_image_data), y_communicate_image_data)
        )
        communicate_image_data.add_precondition(
            have_image(
                r_communicate_image_data,
                o_communicate_image_data,
                m_communicate_image_data,
            )
        )
        communicate_image_data.add_precondition(
            visible(x_communicate_image_data, y_communicate_image_data)
        )
        ...
        communicate_image_data.add_effect(
            communicated_image_data(o_communicate_image_data, m_communicate_image_data),
            True,
        )
        ...
        for i in range(d):
            ...
            if dim == "2D":
                door_closed_configs.append(
                    ConfigurationObject(
                        f"door{i}_closed", DoorConfig, SE2(56.16 + i * 112.25, 52.0, 0)
                    )
                )
                door_open_configs.append(
                    ConfigurationObject(
                        f"door{i}_open", DoorConfig, SE2(76.16 + i * 112.25, 52.0, 0)
                    )
                )
                starts.append(
                    ConfigurationObject(
                        f"start{i}", Waypoint, SE2(20.0 + i * 112.25, 120.0, 0)
                    )
                )
        ...
        navigate = InstantaneousMotionAction(
            "navigate", r=Rover, x=Waypoint, y=Waypoint
        )
        ...
        navigate.add_motion_constraint(
            Waypoints(
                r_navigate,
                x_navigate,
                [y_navigate],
                {door: door_at(door) for door in doors},
            )
        )
        ...
        for i in range(int(d / 2)):
            if dim == "2D":
                soil_pose = SE2(56.16 + i * 2 * 112.25, 20.0, 0)
                rock_pose = SE2(168.4 + i * 2 * 112.25, 20.0, 0)
            if dim == "3D":
                soil_pose = SE3(-18 - 37 * i, -23.0, 0, 0, 0, 0, 0)
                rock_pose = SE3(18 + 37 * i, -23.0, 0, 0, 0, 0, 0)
                ...
                if dim == "2D":
                    ...
                    all_x_images[i] = [
                        ConfigurationObject(
                            f"x_img{i}{0}_pose",
                            Waypoint,
                            SE2(soil_pose.x - 10.0, soil_pose.y + 10.0, 0),
                        ),
                        ConfigurationObject(
                            f"x_img{i}{1}_pose",
                            Waypoint,
                    ...
                    all_y_images[i] = [
                        ConfigurationObject(
                            f"y_img{i}{0}_pose",
                            Waypoint,
                            SE2(rock_pose.x - 10.0, rock_pose.y + 10.0, 0),
                        ),
                        ConfigurationObject(
                            f"y_img{i}{1}_pose",
                            Waypoint,
                ...
                if dim == "3D":
                    ...
                    all_x_images[i] = [
                        ConfigurationObject(
                            f"x_img{i}{0}_pose",
                            Waypoint,
                            SE3(soil_pose.x - 7.0, soil_pose.y + 3.0, 0, 0, 0, 0, 0),
                        ),  # top left
                        ConfigurationObject(
                            f"x_img{i}{1}_pose",
                            Waypoint,
                    ...
                    all_y_images[i] = [
                        ConfigurationObject(
                            f"y_img{i}{0}_pose",
                            Waypoint,
                            SE3(rock_pose.x - 7.0, rock_pose.y + 3.0, 0, 0, 0, 0, 0),
                        ),
                        ConfigurationObject(
                            f"y_img{i}{1}_pose",
                            Waypoint,
        ...
        if dim == "2D":
            l0 = ConfigurationObject("lander0", Waypoint, SE2(d * 112.25 / 2, 158.7, 0))

        if dim == "3D":
            l0 = ConfigurationObject("lander0", Waypoint, SE3(0, 25, 0, 0, 0, 0, 0))
        ...
        if c > 0:
            ...
            problem.add_objects(
                [value for values in x_images.values() for value in values]
            )
            problem.add_objects(
                [value for values in y_images.values() for value in values]
            )
            problem.add_objects(
                [value for values in x_objectives.values() for value in values]
            )
            problem.add_objects(
                [value for values in y_objectives.values() for value in values]
            )
        else:
            problem.add_actions(
                [
        ...
        for i in range(d):
            ...
            if i * 2 + 1 < len(rovers):
                problem.set_initial_value(
                    equipped_for_soil_analysis(rovers[i * 2]), True
                )
                problem.set_initial_value(
                    equipped_for_rock_analysis(rovers[i * 2 + 1]), True
                )
                ...
                if x_images and x_objectives:
                    ...
                    for x_o in x_objectives[i]:
                        problem.set_initial_value(
                            calibration_target(cameras[i * 2], x_o), True
                        )
                        for x_im in x_images[i]:
                ...
                if y_images and y_objectives:
                    for y_im in y_images[i]:
                        problem.set_initial_value(
                            visible(starts[i * 2 + 1], y_im), True
                        )
                        problem.set_initial_value(
                            visible(y_im, starts[i * 2 + 1]), True
                        )

                    for y_o in y_objectives[i]:
                        problem.set_initial_value(
                            calibration_target(cameras[i * 2 + 1], y_o), True
                        )
                        for y_im in y_images[i]:
